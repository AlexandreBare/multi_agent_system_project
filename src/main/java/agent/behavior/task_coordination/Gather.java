...
//        agentState.prettyPrintWorld();
        ...
        ///////////// Check whether the agent can deliver his packet because he is close enough /////////////
        ...
        if (distAgentGathering <= GATHERING_RADIUS) {
            Coordinate deliverDirection = gatheringCoordinates.diff(agentCoordinates);
            MovementManager movementManager = new MovementManager();
            movementManager.sort(deliverDirection);
            Coordinate deliverCoordinates = movementManager.getMoves().get(0);
            agentAction.putPacket(deliverCoordinates.getX(), deliverCoordinates.getY()); // Put the packet down
            return;
        }
...
//        }
        ...
        if (!agentState.getMemoryFragmentKeys().contains("ShortestPath2Gather")){
            ...
            gatheringCoordinates = new Coordinate(0, 0);
            ...
            gatheringCoordinates.divideBy(packetCoordinatesList.size());
        ...
        if (agentState.getMemoryFragmentKeys().contains("ShortestPath2Gather")){
            ...
            agentCoordinates = agentState.getCoordinates();
